...
\begin{itemize}
    ...
    \item \textbf{Robust stateless fallback.} When the state is lost, corrupted, or exhausted, SHRINCS falls back to a SPHINCS+-based stateless signature (SHRINCS-B/L: 2568 or 4104 bytes, and SHRINCS-B: 3680 bytes). This is still smaller than standard SLH-DSA.
    ...
    \item \textbf{The number of supported signatures}. The construction supports up to 141 (SHRINCS-B), 189 (SHRINCS-L), and 210 (SHRINCS-B32) stateful signatures and up to $2^{20}$ or $2^{32}$ stateless signatures, which should be enough for wallets or Ligthing network.
\end{itemize}
