...
SHRINCS (Shrunken SHPINCS) is a hybrid post-quantum signature scheme that combines the efficiency of stateful signatures with the robustness of stateless ones. It provides with two parameters set depending on what matters for verifier:
    \begin{itemize}
        ...
        \begin{itemize}
            \item SHRINCS-B32: The variant for $2^{32}$ stateless signatures.
        \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item SHRINCS-B/B32: \textbf{only} \texttt{308 + 16q} bytes and \texttt{2042 + q} hashes, which makes this setting practical from the \textit{witness size} perspective
    ...
    \item \textbf{Robust stateless fallback.} When the state is lost, corrupted, or exhausted, SHRINCS falls back to a SPHINCS+-based stateless signature (SHRINCS-B/B32/L: 2568/3680/4104 bytes). This is still smaller than standard SLH-DSA.
    ...
    \item \textbf{The number of supported signatures}. The construction supports up to 141 (-B), 189 (-L), and 210 (-B32) stateful signatures and $2^{20}$ (-B/L) or $2^{32}$ (-B32) stateless signatures.
\end{itemize}
...
\begin{itemize}
    \item That's basically the reason this specification introduced two modes for SHRINCS. -B/B32 is designed for accounting systems, where the user pays for each transaction byte, so the signature size matters. -L is better suited to systems that calculate transaction cost based on the number of computation units consumed. 
