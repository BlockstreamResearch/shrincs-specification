...
\paragraph{Note:} In step 3, \texttt{is\_internal = false} because the stateful UXMSS signs user messages directly (requiring phase 1 + phase 2 hashing). This differs from the stateless XMSS layers which always sign tree roots or PORS+FP public key (\texttt{is\_internal = true}).
...
\begin{verbatim}
    SHRINCS-B:
        ...
        q=140:      32 + 4 + 16*16 + 16*140 =  2548 bytes
        q=141(max): 32 + 4 + 16*16 + 16*141 =  2564 bytes // same as q=140; < 2568 (stateless)
    SHRINCS-L:
        q=1:        32 + 4 + 64*16 + 16 =       1076 bytes
        q=10:       32 + 4 + 64*16 + 16*10 =    1220 bytes  
        q=188:      32 + 4 + 64*16 + 16*188 =   4084 bytes
        q=189(max): 32 + 4 + 64*16 + 16*189 =   4100 bytes // same as q=188; < 4104 (stateless)

    SHRINCS-B32:
        q=1:        32 + 4 + 16*16 + 16 =       308 bytes
        q=10:       32 + 4 + 16*16 + 16*10 =    452 bytes  
        q=209:      32 + 4 + 16*16 + 16*209 =  3652 bytes
        q=210(max): 32 + 4 + 16*16 + 16*210 =  3668 bytes // same as q=209; < 3680 (stateless)
\end{verbatim}
